    ...
    public void exportExcel(List<BankFlow> list, String sheetName, HttpServletResponse response) {
        ...
        try {
            ...
            for (Field field : allFields) {
                if (field.isAnnotationPresent( wgcloud.test1.application.util.Excel.class)) {
            ...
            for (int i = 0; i < fieldArrayList.size(); i++) {
                Field field = fieldArrayList.get(i);
                 wgcloud.test1.application.util.Excel fieldAnnotation = field.getAnnotation( wgcloud.test1.application.util.Excel.class);
